...
const FIELD_ASPECT_RATIO = 2; // Ratio longueur/largeur du terrain
...
export default function MatchReview2D() {
  ...
  // Convertir les coordonnées du jeu en coordonnées SVG
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={styles.fieldContainer}>
        <Svg width={FIELD_WIDTH} height={FIELD_HEIGHT} style={styles.field}>
          ...
          {currentPosition && (
            <Circle
              cx={toSvgX(currentPosition.x)}
              cy={toSvgY(currentPosition.y)}
              r={5}
              fill={theme.primary}
            />
          )}
