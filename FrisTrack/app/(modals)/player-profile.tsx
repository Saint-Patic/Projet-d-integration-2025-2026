...
export default function PlayerProfilScreen() {
	...
	if (isLoading) {
		return (
			<View
				style={[styles.container, { justifyContent: "center", backgroundColor: theme.background }]}
			>
	...
	if (error || !user) {
		return (
			<View
				style={[styles.container, { justifyContent: "center", backgroundColor: theme.background }]}
			>
				<BackButton theme={theme} style={{ position: "absolute", top: 40, left: 20 }} />
				<ThemedText
					style={{
						color: theme.text,
						fontSize: 18,
				...
				<TouchableOpacity
					onPress={() => router.back()}
					style={[styles.errorButton, { backgroundColor: theme.primary, marginTop: 20 }]}
				>
					<ThemedText style={{ color: "#fff", fontWeight: "600" }}>Retour</ThemedText>
				</TouchableOpacity>
			</View>
		);
	}
