	...
	const onDateChange = (event: any, selectedDate?: Date) => {
		...
		if (selectedDate) {
			...
			const age = Math.floor(
				(now.getTime() - selectedDate.getTime()) / (1000 * 60 * 60 * 24 * 365.25)
			);
		...
		try {
			...
			if (form.ageDate) {
				...
				ageNumber = Math.floor(
					(now.getTime() - new Date(form.ageDate).getTime()) / (1000 * 60 * 60 * 24 * 365.25)
				);
			}
			...
			const birthdate = form.ageDate
				...
				: new Date(new Date().getFullYear() - ageNumber, 0, 1).toISOString().split("T")[0];
