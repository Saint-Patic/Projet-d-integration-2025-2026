...
export default function CreateMatchScreen() {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				const coachTeams = allMyTeams.filter((team) => team.coach_id === user.user_id);
				setUserTeams(coachTeams);

				if (coachTeams.length === 0) {
					Alert.alert("Information", "Vous devez être coach d'une équipe pour créer un match.");
				}

				const allTeams = await getTeams();
				const myTeamIds = allMyTeams.map((team) => team.id);
				const opponentTeamsList = allTeams.filter((team) => !myTeamIds.includes(team.id));
	...
	const validateTime = (timeString: string): { valid: boolean; message?: string } => {
	...
	const handleCreateMatch = async () => {
		...
		if (!finalTitle) {
			...
			const opponentTeam = opponentTeams.find((t) => t.id === selectedOpponentTeam);
	...
	const TeamSelector = ({
		...
		<View style={styles.sectionContainer}>
			...
			{error && <ThemedText style={[styles.errorText, { color: "#e74c3c" }]}>{error}</ThemedText>}
		</View>
	);

	const LocalisationSelector = () => (
		<View style={styles.sectionContainer}>
			<ThemedText style={[styles.sectionTitle, { color: theme.text }]}>Lieu</ThemedText>
			<View style={styles.teamsGrid}>
					...
					Localisations.map((Localisation, index) => (
						<TouchableOpacity
							...
							style={[
								...
								selectedLocalisation === Localisation && styles.selectedTeamCard,
							]}
							...
							<View style={{ flex: 1 }}>
								<ThemedText
									style={[
										styles.teamName,
										{
											color: selectedLocalisation === Localisation ? theme.primary : theme.text,
										},
									]}
								>
									{Localisation.city}
								</ThemedText>
								<ThemedText style={[styles.LocalisationDetails, { color: theme.textSecondary }]}>
	...
	const IndoorOutdoorSelector = () => (
		<View style={styles.sectionContainer}>
			<ThemedText style={[styles.sectionTitle, { color: theme.text }]}>Type de terrain</ThemedText>
			<View style={styles.radioContainer}>
				<TouchableOpacity
					style={[
						...
						{
							backgroundColor: theme.surface,
							borderColor: inOutdoor === "outdoor" ? theme.primary : theme.border,
						},
				...
				<TouchableOpacity
					style={[
						...
						{
							backgroundColor: theme.surface,
							borderColor: inOutdoor === "indoor" ? theme.primary : theme.border,
						},
		...
		<ScreenLayout title="Détails du match" headerLeft={<BackButton theme={theme} />} theme={theme}>
			...
			>
				...
				<View style={styles.sectionContainer}>
					...
					/>
					{titleError && (
						<ThemedText style={[styles.errorText, { color: "#e74c3c" }]}>{titleError}</ThemedText>
					)}
				...
				{/* Date */}
				<View style={styles.sectionContainer}>
					<ThemedText style={[styles.sectionTitle, { color: theme.text }]}>Date</ThemedText>
					<TextInput
						style={[
							styles.input,
							{
					...
					/>
					{dateError && (
						<ThemedText style={[styles.errorText, { color: "#e74c3c" }]}>{dateError}</ThemedText>
					)}
					<ThemedText style={[styles.helperText, { color: theme.textSecondary }]}>
				...
				<View style={styles.sectionContainer}>
					...
					/>
					{timeError && (
						<ThemedText style={[styles.errorText, { color: "#e74c3c" }]}>{timeError}</ThemedText>
					)}
					<ThemedText style={[styles.helperText, { color: theme.textSecondary }]}>
				...
				>
					<ThemedText style={styles.createButtonText}>Créer le match</ThemedText>
				</TouchableOpacity>
			</ScrollView>
		</ScreenLayout>
	);
