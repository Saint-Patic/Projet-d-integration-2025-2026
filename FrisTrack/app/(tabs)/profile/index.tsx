...
import { getProfileImage, profilePictures } from "@/components/perso_components/loadImages";
...
import { getDominantHandFromSelection, getDominantHandSelection } from "@/utils/dominantHandUtils";
...
export default function ProfilScreen() {
	...
	useEffect(() => {
		const checkPseudoAvailability = async () => {
			...
			if (form.pseudo.length >= 3 && validatePseudo(form.pseudo)) {
				...
				try {
					const response = await apiClient.get(`${API_URL}/users/check-pseudo/${form.pseudo}`);
	...
	const renderValidationIcon = (error: string, available: boolean | null, isLoading: boolean) => {
		if (isLoading) {
			return <ActivityIndicator size="small" color="#00d6d6" style={styles.validationIcon} />;
		}
		if (error) {
			return (
				<Ionicons name="close-circle" size={20} color="#ff4444" style={styles.validationIcon} />
			);
		}
		if (available === true) {
			return (
				<Ionicons name="checkmark-circle" size={20} color="#00ff88" style={styles.validationIcon} />
			);
		}
		return null;
	};
	...
	const handleSave = async () => {
		...
		// Validation finale
		if (firstnameError || lastnameError || pseudoError || pseudoAvailable === false) {
	...
	const HeaderRight = () => (
		<TouchableOpacity onPress={() => router.push("./profile/settings")} style={{ marginRight: 16 }}>
			<Ionicons name="settings-outline" size={24} color={theme.primary} />
		</TouchableOpacity>
	);

	const HeaderLeft = () => (
		<TouchableOpacity onPress={() => router.push("./profile/notifs")} style={{ marginRight: 16 }}>
			<Ionicons name={"file-tray-full-outline"} size={24} color={theme.primary} />
		</TouchableOpacity>
	);
		...
		return (
			<Pressable style={styles.fullImageOverlay} onPress={() => setShowFullImage(false)}>
				<BackButton
					...
					color="#fff"
				/>
				<TouchableOpacity activeOpacity={1} onPress={() => setShowFullImage(false)}>
					<Image
					...
					>
						...
						{showImagePicker ? (
								...
								<View style={styles.imagePickerGrid}>
									{profilePictures.map((img) => (
										...
										>
											<Image
												source={img.src}
												style={[styles.profileImageSmall, { borderColor: theme.primary }]}
											/>
										</TouchableOpacity>
									))}
								</View>
							...
							<View style={styles.profileImageContainer}>
								<TouchableOpacity onPress={() => setShowImagePicker(true)} activeOpacity={0.7}>
									<Image
						...
						<View style={styles.infoContainer}>
							...
							<View style={styles.inputContainer}>
								...
								/>
								{form.firstname !== "" && renderValidationIcon(firstnameError, null, false)}
							</View>
							{firstnameError !== "" && (
								<ThemedText style={styles.fieldErrorText}>{firstnameError}</ThemedText>
							)}
							...
							<View style={styles.inputContainer}>
								...
								<TextInput
									...
									autoCapitalize="words"
								/>
								{form.lastname !== "" && renderValidationIcon(lastnameError, null, false)}
							</View>
							{lastnameError !== "" && (
								<ThemedText style={styles.fieldErrorText}>{lastnameError}</ThemedText>
							)}
							...
							{pseudoAvailable === true && pseudoError === "" && form.pseudo !== originalPseudo && (
								<ThemedText style={styles.successText}>Pseudo disponible ✓</ThemedText>
							)}
							...
							<View style={[styles.infoRow, { borderBottomColor: theme.border }]}>
								...
								<TextInput
									...
									onChangeText={(text) => {
										...
										if (filtered !== "") setForm({ ...form, foot_size: parseInt(filtered) });
									}}
									onBlur={() => {
							...
							<View style={[styles.infoRow, { borderBottomColor: theme.border }]}>
								...
								<TextInput
									...
									onChangeText={(text) => {
										...
										if (filtered !== "") setForm({ ...form, user_height: parseInt(filtered) });
									}}
									onBlur={() => {
...
const styles = StyleSheet.create({
	...
	sensorButton: {
		backgroundColor: Platform.OS === "android" ? "#00a0a0" : "rgba(0, 184, 184, 0.7)",
		...(Platform.OS === "ios" && {
