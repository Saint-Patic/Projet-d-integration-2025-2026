...
export default function CreateTeamScreen() {
	...
	const [selectedPlayers, setSelectedPlayers] = useState<UserForSelection[]>(
		[],
	);
	...
	const handleUserSelect = (userItem: UserForSelection) => {
		...
		const isAlreadySelected = selectedPlayers.some(
			(p) => p.user_id === userItem.user_id,
		);
		if (isAlreadySelected) {
			setSelectedPlayers(
				selectedPlayers.filter((p) => p.user_id !== userItem.user_id),
			);
		} else {
			setSelectedPlayers([...selectedPlayers, userItem]);
		}
	};
	...
	const removePlayer = (userId: number) => {
		setSelectedPlayers(selectedPlayers.filter((p) => p.user_id !== userId));
	};
	...
	const handleCreateTeam = async () => {
		...
		try {
			...
			Alert.alert(
				"Succès",
				`L'équipe "${result.team_name}" a été créée avec succès`,
				[
					{
						text: "OK",
						onPress: () => router.back(),
	...
	if (showUserSelection) {
		return (
			<ScreenLayout
				...
				headerRight={
					<TouchableOpacity
						onPress={confirmPlayerSelection}
							...
							>
								<View style={styles.userInfo}>
									...
									<View style={styles.userDetails}>
										...
										<ThemedText
											style={[styles.userType, { color: theme.textSecondary }]}
										>
			...
			>
					...
					>
						<View style={styles.userInfo}>
							...
							<View style={styles.userDetails}>
								...
								{user?.pseudo && (
									<ThemedText
										style={[styles.userPseudo, { color: theme.textSecondary }]}
									>
										@{user.pseudo}
									</ThemedText>
								)}
								<ThemedText
									style={[styles.coachBadge, { color: theme.primary }]}
								>
									Vous êtes le coach
								</ThemedText>
							</View>
				...
				<View style={styles.section}>
					...
					>
						...
						<ThemedText
							style={[styles.selectButtonText, { color: theme.primary }]}
						>
							Ajouter des joueurs
						</ThemedText>
					</TouchableOpacity>
