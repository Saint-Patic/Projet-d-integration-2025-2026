...
export default function CreateTeamScreen() {
  ...
  const [confirmedPlayers, setConfirmedPlayers] = useState<UserForSelection[]>(
    []
  ); // Joueurs confirmés
  const [tempSelectedPlayers, setTempSelectedPlayers] = useState<
    UserForSelection[]
  >([]); // Sélection temporaire pendant le modal
  ...
  const handleUserSelect = (userItem: UserForSelection) => {
    ...
    const isAlreadySelected = tempSelectedPlayers.some(
      (p) => p.user_id === userItem.user_id
    );
    if (isAlreadySelected) {
      setTempSelectedPlayers(
        tempSelectedPlayers.filter((p) => p.user_id !== userItem.user_id)
      );
    } else {
