...
export default function TeamDetailsScreen() {
	...
	useEffect(() => {
		...
		const unsubscribe = navigation.addListener("beforeRemove", (e) => {
			if (
				!isEditMode ||
				(members.length === originalMembers.length && playersToRemove.length === 0)
			) {
	...
	const handleAddPlayer = () => {
		if (!isCoach) {
			Alert.alert("Permission refusée", "Seul le coach peut ajouter des joueurs");
			return;
		}
		router.push({
			pathname: "./add-player",
	...
	const performSaveChanges = async (changedMembers: Member[]) => {
		...
		if (errorCount === 0) {
			...
			setPlayersToRemove([]);
			setIsEditMode(false);
			Alert.alert("Attention", "Certaines modifications n'ont pas pu être enregistrées");
		}
	};
				...
				<View style={styles.headerRow}>
					...
					{isCoach && (
						<View style={[styles.coachBadge, { backgroundColor: theme.primary }]}>
							...
							<ThemedText style={styles.coachBadgeText}>Vous êtes coach</ThemedText>
						</View>
					)}
				</View>
